# We need to import sys  to stope the code in the
# middle if the inputs are not right
"""Need to import sys to stop the code in the middle if someting not right"""
import sys
...
def prepared_for_rain(have_umbrella, rain_level, have_hood, is_workday):
    """A function that checks are you ready for a rainy day"""
    output = (
        have_umbrella
...
def get_bool(input_value):
    """A function that takes input string True or False and returns bool True
    or False"""
    output = None
    if input_value == "True":
        output = True
...
rain_preparation = prepared_for_rain(umbrella_status, rain_status, hood_status,
                                     workday_status)
